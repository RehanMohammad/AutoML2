...
from .forms import DataUploadForm, ModelTrainingForm, SUBMITFORTrainingForm
import pandas as pd
...
def home():
    ...
    if form.validate_on_submit():
        ...
        filepath = os.path.join(current_app.config['UPLOAD_FOLDER'], form.file.data.filename)
        form.file.data.save(filepath)
        x=form.x_columns.data.split(',')
        y=form.y_column.data
        # df = pd.read_csv(form.file.data.filename) 
        return redirect(url_for('main.visualize', filepath=filepath,x=x , y=y))
...
def visualize():
    ...
    if form.validate_on_submit():  
        return redirect(url_for('main.train', filepath=filepath, x=x, y=y))
...
def train():    
    ...
    if form.validate_on_submit():
        ...
        post = Post(body=automl.report(), author=current_user._get_current_object())
        db.session.add(post)
        db.session.commit()
